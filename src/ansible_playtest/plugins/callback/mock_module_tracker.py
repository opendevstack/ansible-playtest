...
DOCUMENTATION = '''
    ...
    version_added: "1.0"
    description:
        - This callback tracks module executions, especially for mocked modules.
...
class CallbackModule(CallbackBase):
    ...
    def _track_module_call(self, module_name, result):
        ...
        call_info = {
            'params': params,
            'result': result_data,
            'task': result._task.name if hasattr(result, '_task') and hasattr(result._task, 'name') else 'unnamed_task',
    ...
    def _save_summary_to_cwd(self, stats):
        ...
        try:
            ...
            if hasattr(self, '_playbook_name') and self._playbook_name:
                playbook_name = os.path.basename(self._playbook_name).replace('.yml', '').replace('.yaml', '')
            ...
            total_changed = sum(host_stats['changed'] for host_stats in hosts_stats.values())
            total_unreachable = sum(host_stats['unreachable'] for host_stats in hosts_stats.values())
            total_failed = sum(host_stats['failures'] for host_stats in hosts_stats.values())
            total_skipped = sum(host_stats['skipped'] for host_stats in hosts_stats.values())
            total_rescued = sum(host_stats['rescued'] for host_stats in hosts_stats.values())
            total_ignored = sum(host_stats['ignored'] for host_stats in hosts_stats.values())
            ...
            summary_data = {
                ...
                'total_failures': sum(self.failed_modules.values()) if self.failed_modules else 0,
                'skipped_modules': self.skipped_modules,
                'total_skipped': sum(self.skipped_modules.values()) if self.skipped_modules else 0,
            ...
            self._display.display(f"Module calls summary saved to {save_location}: {summary_path}", color='green')
            return summary_path
    ...
    def v2_runner_on_failed(self, result, ignore_errors=False):
        ...
        self._display.display(f"Module failure: {module_name} - {error_msg}", color='red')
