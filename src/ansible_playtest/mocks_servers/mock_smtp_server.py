...
# Configure logging - default to ERROR level for quieter output
...
class MockSMTPHandler:
    ...
    async def handle_DATA(self, server, session, envelope):
        ...
        logger.info(f"Received message from {envelope.mail_from} to {envelope.rcpt_tos} - Subject: {subject}")
...
class MockSMTPServer:
    ...
    def run_command_line(cls):
        ...
        parser = argparse.ArgumentParser(description='Run a mock SMTP server for testing')
                ...
                if args.verbose >= 1:
                    ...
                    if messages:
                        ...
                        for i, msg in enumerate(messages, 1):
                            logger.info(f"  {i}. From: {msg['mail_from']} To: {msg['rcpt_tos']}")
