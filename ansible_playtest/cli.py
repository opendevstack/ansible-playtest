...
def main():
    ...
    parser.add_argument('--config-dir', '-c', help='Directory containing scenarios and test data (overrides ANSIBLE_PLAYTEST_CONFIG_DIR environment variable)')
    ...
    smtp_group.add_argument('--start-smtp-mock', action='store_true', help='Start the mock SMTP server before running the playbook')
    ...
    try:
        ...
        if not os.path.exists(args.playbook):
            ...
            print("Usage example:")
            print("  ansible-playtest --scenario examples/scenarios/example_scenario.yaml ./examples/playbooks/demo_playbook_01.yml")
            ...
            if not scenario_found:
                            ...
                            for file in files:
                                if file.endswith('.yaml') or file.endswith('.yml'):
                                    if scenario_name.lower() in file.lower():
                                        unique_matches.add(os.path.normpath(os.path.join(root, file)))
                ...
                # If multiple fuzzy matches found with different basenames, fail with a helpful error
                    ...
                    if len(basenames) == 1:
                        ...
                        scenario_path = fuzzy_matches[0]
                        scenario_found = True
                        print(f"Found multiple copies of the same scenario file. Using: {scenario_path}")
                    else:
                        print(f"Error: Multiple different scenarios found matching '{scenario_name}':")
        ...
        if not scenario_found:
            ...
            if not config_dir:
                config_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'tests', 'test_data', 'scenarios')
            print(f"Looking for scenarios in: {config_dir}")
            if os.path.isdir(config_dir):
                scenario_files = [f for f in os.listdir(config_dir) if f.endswith('.yaml') or f.endswith('.yml')]
                if scenario_files:
            ...
            print("\nUsage example:")
            print("  ansible-playtest --scenario <scenario_path> ./examples/playbooks/demo_playbook_01.yml")
            print("\nSee INSTALL.md for more details.")
            sys.exit(2)
