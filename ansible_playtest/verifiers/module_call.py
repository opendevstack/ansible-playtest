...
class ModuleCallCountVerifier(VerificationStrategy):
    ...
    def verify(self, scenario_data, playbook_stats):
        ...
        verification_results["_overall_pass"] = (
            all(result["passed"] for result in verification_results.values())
    ...
    def _print_call_count_results(self, verification_results):
        ...
        for module_name, result in verification_results.items():
            ...
            if not result["passed"]:
                ...
                if diff > 0:
                    print(
                        f"  {self.RED}Error: Module called {diff} more times than expected{self.RESET}"
                    )
                else:
                    print(
                        f"  {self.RED}Error: Module called {abs(diff)} fewer times than expected{self.RESET}"
                    )
